...
def query_list(project):
  ...
  queries = [
    ...
    SELECT 
     ...
     severity,
     insertId,
    FROM `{project}.log_sink.cloudaudit_googleapis_com_activity_20231122` 
    LIMIT 3;
    """,
    f"""
    SELECT 
...
def slack_push():
    ...
    upload_text_file = slack_client.files_upload(
        channels="#general",
        title=f"Bigquery combined result",
        file=pdf_file_name,
        #initial_comment=f"{report_name} file:",
    )
    print(f"Combined result pushed to slack channel")



def main():
        query_to_pdf(projects)
        slack_push()    
